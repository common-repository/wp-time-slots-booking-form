...
class CP_TimeSlotsBookingPlugin extends CP_TSLOTSBOOK_BaseClass {
   ...
   function data_management_loaded() {
    ...
    function data_management() {
        ...
        foreach ($my_POST as $item => $value)
            if (isset($fields[str_replace($sequence,'',$item)]))
            {
                ...
                if (!in_array(str_replace($sequence,'',$item), $excluded_items))
                    $buffer .= $fields[str_replace($sequence,'',$item)] . ": ". $this->filter_allowed_tags(is_array($value)?(implode(", ",$value)):($value)) . "\n\n";
                $params[str_replace($sequence,'',$item)] = $this->filter_allowed_tags($value);
            }
        ...
        // clear captcha
        $_SESSION['rand_code'.$sequence] = '';
        if (!empty($_COOKIE['rand_code'.$sequence])) set_transient( "cpeople-captcha-".sanitize_key($_COOKIE['rand_code'.$sequence]) , '' , 1800 ); 
        ...
        foreach($form as $field)
            if ($field->ftype == 'fslots' && @$data[$field->name.$sequence] != '')
            {
                ...
                foreach($apps_text as $app_item_text)
                {
                    ...
                    $subid++;
                    $server_side_price = $this->get_slot_price($field,
                                                               $item_split[0] , $item_split[1], intval($item_split[8]),
                                                               $quantity1, $quantity2, $quantity3, $quantity4, $quantity5, count($apps_text)); 
                    $apps[] = array (
                                     'id' => $subid,
    ...
    {   
        return floatval($field->prices[$priceindex]->price1[$totalslots])*intval($quantity1) + 
               ...
               ( property_exists( $field->prices[$priceindex], 'price4') ? floatval($field->prices[$priceindex]->price4[$totalslots])*intval($quantity4) : 0 ) +
               ( property_exists( $field->prices[$priceindex], 'price5') ? floatval($field->prices[$priceindex]->price5[$totalslots])*intval($quantity5) : 0 );
        exit;
    }
        ...
        foreach($apps as $app)
            if ($itemname == '' || $itemname == $app["field"])        
            {
                ...
                $text .= " - ".$this->format_date($app["date"])." ".$slot.
				    ($app["quantity1"] + $app["quantity2"] + $app["quantity3"] + $app["quantity4"] + $app["quantity5"] > 1 || $app["quantity1"] == 0 ? 
				      " (".(
    ...
    function save_options()
    {
        ...
        $data = array(
                      'form_structure' => $this->clean_sanitize($this->admin_process_json($my_POST['form_structure'])),
        ...
        foreach ($events as $item)
        {
            ...
            for ($i=0; $i<$end; $i++)
                {  //  if (isset($data[$fields[$i]]) )
                    ...
                    {
                       if (isset($data["apps"]) && isset($data["apps"][ intval( substr($fields[$i],strlen('app_status_'))-1) ]) && isset($data["apps"][ intval( substr($fields[$i],strlen('app_status_'))-1) ]["cancelled"]))
                ...
                switch ($fields[$j]) {
                    ...
                    default:
                    {
                        if (isset($selection[$i][3][$fields[$j]]))
                            $value = $this->clean_csv_value($selection[$i][3][$fields[$j]]==''?'':$selection[$i][3][$fields[$j]])."";
                        else
                            $value = '';
                    }
                } 
